  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4">
        <Activity className="w-8 h-8 animate-spin text-brand-600 mb-2" />
        <p className="text-xs text-slate-500">Verifying secure medical token...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 py-4 px-6">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-brand-600 to-cyan-400 flex items-center justify-center text-white">
              <Activity className="w-5 h-5" />
            </div>
            <span className="font-heading font-bold text-base text-slate-900 dark:text-white">
              HEALTHMATE AI <span className="text-xs text-brand-600 font-normal ml-1">Consultation Portal</span>
            </span>
          </div>

          <div className="flex items-center space-x-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 text-xs font-medium">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {!records ? (
          <div className="max-w-md mx-auto mt-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-xl space-y-6 text-center">
            ...
            {error && (
              <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-300 text-xs flex items-center space-x-2 text-left">
            ...
            <form onSubmit={handlePinSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="••••"
                  className="w-full text-center tracking-widest text-lg px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500 text-slate-900 dark:text-white font-mono"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-brand-600 to-cyan-600 hover:from-brand-700 hover:to-cyan-700 text-white text-xs font-semibold rounded-xl shadow-md transition-all flex items-center justify-center space-x-2"
              >
                {isUnlocking ? (
                  <>
            ...
            <div className="p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-brand-50 dark:bg-brand-950 text-brand-700 dark:text-brand-300">
                      ...
                      <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {doc.prescriptions.map((p, idx) => (
                            <div key={idx} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 text-xs">
                              <p className="font-bold text-slate-900 dark:text-white">{p.medication_name}</p>
                              <p className="text-slate-500">{p.dosage} • {p.frequency} • {p.timing_instructions}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {doc.lab_tests.map((l, idx) => (
                            <div key={idx} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 text-xs flex justify-between items-center">
                              <div>
                                <p className="font-bold text-slate-900 dark:text-white">{l.test_name}</p>
                                <p className="text-slate-500">{l.observed_value} {l.unit} [Ref: {l.reference_range_text || 'Standard'}]</p>
                              </div>
